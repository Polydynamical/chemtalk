...
text = input("Text to translate: ").lower()

arr = []
s = 0
e = 1
while e <= len(text):
    if text[s:e] in symbols_lower:
        arr.append(symbols[symbols_lower.index(text[s:e])])
        s += 1
        e += 1
        continue

    if text[s:e + 1] in symbols_lower:
        arr.append(symbols[symbols_lower.index(text[s:e+1])])
        s += 2
        e += 2
        continue

    arr.append(f"({text[s:e]})")
    s += 1
    e += 1
